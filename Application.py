...
APPLICATION_NAME = 'EventsApplication'
...
# --------------------- gconnect/gdisconnect via google account ---------------------------
...
# 1- return one item for one Catalog in JSON format
...
def ItemJSON(catalog_id, item_id):
    ...
    return jsonify(ItemsJson=items.serialize)


# 2- return items for one catalog in JSON format
...
def newItem(catalog_id):
    ...
    if 'username' not in login_session:
        return '''<script>function myFunction() { alert('You must login to add events.');} </script><body onload='myFunction()''>'''
...
def editItem(catalog_id, item_id):
    ...
    if login_session['user_id'] != editedItem.user_id:
        return '''<script>function myFunction() {alert('You are not allowed to eidt this item.');}</script><body onload='myFunction()''>'''
    if request.method == 'POST':
...
def deleteItem(catalog_id, item_id):
    ...
    if login_session['user_id'] != deletedItem.user_id:
        return '''<script>function myFunction() {alert('You are not allowed to delete items.');}</script><body onload='myFunction()''>'''
    if request.method == 'POST':
